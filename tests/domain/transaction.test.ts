...
describe("Transaction", () => {
  describe("create()", () => {
    ...
    it("creates a pre-categorized transaction when categoryId is provided", () => {
      const txn = Transaction.create({ ...BASE_PARAMS, categoryId: CategoryId.from("n01") });
      expect(txn.categoryId?.value).toBe("n01");
    });
  ...
  describe("categorize()", () => {
    it("returns a new Transaction with the given categoryId", () => {
      const txn = Transaction.create(BASE_PARAMS);
      const categorized = txn.categorize(CategoryId.from("n02"));
      expect(categorized.categoryId?.value).toBe("n02");
    });

    it("preserves all other fields", () => {
      const txn = Transaction.create(BASE_PARAMS);
      const categorized = txn.categorize(CategoryId.from("n02"));
      expect(categorized.id.equals(txn.id)).toBe(true);
    ...
    it("does not mutate the original", () => {
      const txn = Transaction.create(BASE_PARAMS);
      txn.categorize(CategoryId.from("n02"));
      expect(txn.categoryId).toBeUndefined();
    });

    it("re-categorization replaces the categoryId", () => {
      const txn = Transaction.create({ ...BASE_PARAMS, categoryId: CategoryId.from("n01") });
      const recategorized = txn.categorize(CategoryId.from("w02"));
      expect(recategorized.categoryId?.value).toBe("w02");
      expect(txn.categoryId?.value).toBe("n01"); // original unchanged
    });

    it("returns a Transaction instance", () => {
      const txn = Transaction.create(BASE_PARAMS);
      expect(txn.categorize(CategoryId.from("n01"))).toBeInstanceOf(Transaction);
    });
  });
  ...
  describe("isCategorized", () => {
    ...
    it("returns true after categorize()", () => {
      const txn = Transaction.create(BASE_PARAMS);
      expect(txn.categorize(CategoryId.from("n01")).isCategorized).toBe(true);
    });
  });
